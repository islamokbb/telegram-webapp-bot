...
bot.on("message", async msg => {
  ...
  // ===== أزرار عامة (تلغي أي وضع) =====
  if (t === "📰 أوراق اليوم") {
    STATE.set(id, "NONE");
    const bets = JSON.parse(fs.readFileSync(BETS_FILE));
    if (!bets.length) return bot.sendMessage(id, "📭 لا توجد رهانات اليوم");
...
أنت موقع مراهنات كرة قدم محترف.
أعطني توقعات رهانات فقط بدون أي شرح.

المباراة:
${t}

اكتب بهذا القالب فقط:

🏆 الفائز الأقرب: (اسم الفريق + ✅)
...
ممنوع إضافة أي كلام خارج هذا القالب.
`);
