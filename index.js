...
bot.on("message", async msg => {
  ...
  if (t === "🤖 تحليل رياضي AI") {
    ...
    return bot.sendMessage(id,"🧠 اكتب أي سؤال تحليلي (استمر حتى تضغط إيقاف)");
  }

  if (STATE.get(id) === "ANALYZE" && t !== "❌ إيقاف التحليل") {
    bot.sendChatAction(id,"typing");
    return bot.sendMessage(
      id,
      await askAI(`حلل رياضيًا:\n${t}`)
    );
  }
...
`);
  ...
  if (t === "❌ إيقاف التحليل") {
    STATE.set(id,"NONE");
    return bot.sendMessage(id,"تم إيقاف التحليل");
  }
